...
var solutionFileName = "FacilityAspNet.sln";
var docsAssemblies = new[] { "Facility.CodeGen.AspNet" };
var docsAssemblyDirectory = $"src/fsdgenaspnet/bin/{configuration}";
var docsRepoUri = "https://github.com/FacilityApi/FacilityApi.github.io.git";
var docsRepoBranch = "master";
var docsSourceUri = "https://github.com/FacilityApi/FacilityAspNet/tree/master/src";
...
Task("Test")
	...
	.Does(() =>
	{
		foreach (var projectPath in GetFiles("tests/**/*.UnitTests.csproj").Select(x => x.FullPath))
...
{
	ExecuteCodeGen($@"{File("example/ExampleApi.fsd")} {Directory("example/aspnet")}/", verify);
}

void ExecuteCodeGen(string args, bool verify)
{
	string exePath = File($"src/fsdgenaspnet/bin/{configuration}/fsdgenaspnet.exe");
	if (IsRunningOnUnix())
	{
		args = exePath + " " + args;
		exePath = "mono";
